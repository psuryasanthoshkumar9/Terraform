      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="flex items-center gap-4">
            <button onClick={()=>setShowAdd(!showAdd)} className="ml-auto bg-indigo-600 text-white px-4 py-2 rounded-lg shadow">+ Add Student</button>
          </div>
        </div>
        ...
        {showAdd && (
          <form className="bg-white p-4 rounded-lg shadow mb-6" onSubmit={handleAdd}>
            <div className="flex gap-4">
              <input value={form.name} onChange={e=>setForm(s=>({...s,name:e.target.value}))} placeholder="Name" className="border p-2 rounded w-1/3"/>
              <input value={form.age} onChange={e=>setForm(s=>({...s,age:e.target.value}))} placeholder="Age" className="border p-2 rounded w-1/6"/>
              <input value={form.class} onChange={e=>setForm(s=>({...s,class:e.target.value}))} placeholder="Class" className="border p-2 rounded w-1/6"/>
              <button type="submit" className="bg-green-600 text-white px-4 rounded">Save</button>
            </div>
          </form>
        )}
              ...
              {students.map(s => (
                <div key={s.id} className="flex items-center justify-between border rounded p-3">
                  <div>
                    ...
                    <div className="text-sm text-slate-500">Age: {s.age} • Class: {s.class}</div>
                  </div>
